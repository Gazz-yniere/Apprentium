...
# Import pour pouvoir wrapper lxml element
...
warnings.filterwarnings(
    "ignore", message="style lookup by style_id is deprecated")
...
def get_output_path(filename, custom_output_dir=None):
    ...
    if base_dir_for_output is None:  # Si custom_output_dir n'est pas fourni ou a échoué
        if getattr(sys, 'frozen', False):  # Exécutable PyInstaller
            ...
            # Remonte au dossier parent de src/
            app_base_path = os.path.dirname(os.path.dirname(__file__))
        base_dir_for_output = os.path.normpath(
            os.path.join(app_base_path, 'output'))

    # S'assure que le dossier final existe
    ...
    for idx, problem_data in enumerate(problems_for_day):
        ...
        if idx < len(problems_for_day) - 1:
            p_answer.paragraph_format.space_after = Pt(
                6)  # Espace avant le prochain problème
        else:
...
def set_table_borders_visible_colored(table, rgb_float_color, size_pt=6):
    ...
    for border_name in ['top', 'left', 'bottom', 'right']:
        ...
        # Taille en 1/8 de point (ex: 6 pour 0.75pt)
        border_el.set(qn('w:sz'), str(size_pt))
        border_el.set(qn('w:space'), '0')
...
def add_section_header_word(parent_cell, section_key_name, section_num_val):
    ...
    if image_path_relative:
        # Utilise la fonction de pdf_generator
        image_path_abs = get_resource_path_pdf(image_path_relative)
        if image_path_abs and os.path.exists(image_path_abs):
            try:
                img_para.add_run().add_picture(
                    image_path_abs, width=Inches(0.4))  # Taille de l'image réduite
...
def delete_paragraph_by_element(p_element, parent_docx_object):
    ...
    if p_element is None or p_element.getparent() is None:
        return False
    try:
        ...
        print(
            f"Erreur lors de la vérification/suppression du paragraphe par élément : {e}")
        pass
    return False
...
def set_cell_margins(cell, top=None, bottom=None, left=None, right=None):
    ...
    for margin_name, value_pt in margins_to_set.items():
        if value_pt is not None:
            ...
            # Convertir points en DXA (twips)
            margin_sub_element.set(qn("w:w"), str(int(value_pt.pt * 20)))
...
def generate_workbook_docx(days, operations, counts, max_digits, conjugations, params_list, grammar_exercises,
    ...
    if english_exercises is None:
        english_exercises = []
    if orthographe_exercises is None:
        orthographe_exercises = []
    if enumerate_exercises is None:
        enumerate_exercises = []
    ...
    # Un peu plus de marge à gauche pour l'ensemble
    ...
    def add_paragraph(parent_container, text="", style=None, indent=False):
        para = parent_container.add_paragraph(
            text, style=style) if style else parent_container.add_paragraph(text)
        para.paragraph_format.space_before = Pt(
            0)  # Assurer aucun espace avant
    ...
    for day in range(1, days + 1):
        ...
        if header_text or show_name or show_note:
            ...
            if show_note:
                ...
                row.cells[2].text = ""
            # Espace après l'en-tête général, ajouté au document principal
            add_paragraph(doc, "")
        ...
        if any(counts) or \
           ...
           bool(current_day_story_problems):  # Condition pour inclure les "Petits Problèmes"
            ...
            section_color_data = SECTION_ASSETS.get(
                section_key, {}).get("color", (0.1, 0.1, 0.1))
            set_table_borders_visible_colored(
                section_frame_table, section_color_data, size_pt=8)  # Cadre visible

            section_cell = section_frame_table.cell(0, 0)
            set_cell_margins(section_cell, top=Pt(0), bottom=Pt(
                10), left=Cm(0.15), right=Cm(0.15))
            if section_cell.paragraphs:  # Nettoyer le paragraphe par défaut
                p_default = section_cell.paragraphs[0]
                if not p_default.text and not p_default.runs:
                    delete_paragraph(p_default)
            ...
            if enumerate_exercises and len(enumerate_exercises) >= day and enumerate_exercises[day-1]:
                ...
                run_enum = para_enum.add_run(
                    "Écris chaque nombre en toutes lettres :")
                run_enum.bold = True
                for n_enum in enumerate_exercises[day-1]:
                    add_paragraph(
                        section_cell, f"{n_enum} = _____________________________________________", style='ListContinue', indent=True)
                ...
                for i, operation in enumerate(operations):
                    ...
                    if params_list[i].get('count', 0) > 0:
                        ...
                        problems = generate_arithmetic_problems(
                            operation, params_list[i])  # Utilise la fonction importée
                        for problem in problems:
                            calc_str = problem.strip().replace(' =', '')
                            add_paragraph(
                                section_cell, f"{calc_str} = ________________________________", style='ListContinue', indent=True)
        ...
        if current_day_conjugations or current_day_complete_sentence or current_day_complete_pronoun:
            ...
            section_color_data = SECTION_ASSETS.get(
                section_key, {}).get("color", (0.1, 0.1, 0.1))
            set_table_borders_visible_colored(
                section_frame_table, section_color_data, size_pt=8)
            section_cell = section_frame_table.cell(0, 0)
            set_cell_margins(section_cell, top=Pt(0), bottom=Pt(
                10), left=Cm(0.15), right=Cm(0.15))
        ...
        if grammar_exercises and len(grammar_exercises) >= day and grammar_exercises[day-1]:
            ...
            for ex in grammar_exercises[day-1]:
                ...
                transfo_label_run = transfo_line_para.add_run(
                    "Transformation demandée : ")
                transfo_label_run.bold = True
                transfo_line_para.add_run(transformation_content)

                add_paragraph(
                    section_cell, "Réponse : __________________________________________________________", indent=True)
            add_paragraph(doc, "")  # Espace entre les cadres de section
        ...
        if has_mesures_content_for_day:
            ...
            if geo_exercises and len(geo_exercises) >= day and geo_exercises[day-1]:
                ...
                if current_day_geo_ex:
                    ...
                    if len(current_day_geo_ex) == 1:
                        run_conv_title = para_conv_title.add_run(
                            "Conversion :")
                    else:
                        run_conv_title = para_conv_title.add_run(
                            "Conversions :")
            ...
            if sort_exercises and len(sort_exercises) >= day and sort_exercises[day-1]:
                ...
                if current_day_sort_ex:
                    ...
                    run_sort_title = para_sort_title.add_run(
                        f"Range les nombres suivants dans l'ordre {ordre} :")
                    run_sort_title.bold = True
                    for ex_sort in current_day_sort_ex:
                        numbers_str = ", ".join(str(n)
                                                for n in ex_sort['numbers'])
                        add_paragraph(
                            section_cell, f"{numbers_str} = _________________________________", indent=True)
            if current_day_encadrement_lines:
                ...
                if len(current_day_encadrement_lines) == 1:
                    run_enc_title = para_enc_title.add_run(
                        "Encadre le nombre :")
                else:
                    run_enc_title = para_enc_title.add_run(
                        "Encadre les nombres :")
                ...
                for ex_enc in current_day_encadrement_lines:
                    ...
                    label = f"à l'{t_enc}" if t_enc == "unité" else f"à la {t_enc}" if t_enc in [
                        "dizaine", "centaine"] else f"au {t_enc}"
                    add_paragraph(
                        section_cell, f"{n_enc} {label} : ______  {n_enc}  ______", indent=True)
            add_paragraph(doc, "")  # Espace entre les cadres de section
            ...
            if current_day_compare_numbers:
                ...
                for ex_compare in current_day_compare_numbers:
                    add_paragraph(
                        section_cell, f"{ex_compare['num1']} ______ {ex_compare['num2']}", indent=True)
        ...
        if english_exercises and len(english_exercises) >= day and english_exercises[day-1]:
            ...
            for ex in english_exercises[day-1]:
                if ex['type'] in ('simple', 'complexe'):
                    ...
                    para_relier_title = add_paragraph(
                        section_cell, indent=True)
                    run_relier_title = para_relier_title.add_run(
                        "Jeu de mots à relier :")
                    run_relier_title.bold = True

                    mots_a_relier_pour_jeu = ex['content']
                    liste_anglais = [item['english']
                                     for item in mots_a_relier_pour_jeu]
                    liste_francais = [item['french']
                                      for item in mots_a_relier_pour_jeu]
                    ...
                    if max_items > 0:
                        ...
                        table = section_cell.add_table(
                            rows=max_items, cols=6)  # Pré-allouer les lignes
                        table.autofit = False
                        ...
                        # Largeur doublée pour les colonnes de mots
                        col_word_width = Inches(2)
                        col_bullet_width = Inches(0.1)
                        col_space_width = Inches(0.2)

                        target_table_width_inches = col_indent_width.inches + \
                            (col_word_width.inches * 2) + \
                            (col_bullet_width.inches * 2) + \
                            col_space_width.inches
                        target_table_width_dxa = int(
                            target_table_width_inches * 1440)
                        ...
                        table.columns[0].width = col_indent_width
                        # Mot anglais
                        table.columns[1].width = col_word_width
                        table.columns[2].width = col_bullet_width   # Puce 1
                        # Espace central
                        table.columns[3].width = col_space_width
                        table.columns[4].width = col_bullet_width   # Puce 2
                        # Mot français
                        table.columns[5].width = col_word_width

                        for i in range(max_items):
                            mot_anglais = liste_anglais[i]
                            mot_francais = liste_francais[i]

                            # Obtenir les cellules de la ligne courante
                            row_cells = table.rows[i].cells

                            contents = ["", mot_anglais, "\u2022",
                                        "", "\u2022", mot_francais]
        ...
        if orthographe_exercises and len(orthographe_exercises) >= day and orthographe_exercises[day-1]:
            ...
            for ex_ortho in orthographe_exercises[day-1]:
                if ex_ortho['type'] == 'homophone':
                    ...
                    run_homo_type = para_homo.add_run(
                        f"{ex_ortho['homophone']} : ")
